...
from core.generate import (
    ...
    generate_base_image_seedream4_replicate,
    edit_base_image_seedream4_replicate,
    generate_base_image_seedream4_bytedance,
    edit_base_image_seedream4_bytedance,
    poll_image_seedream4_bytedance,
)
    ...
    while True:
        ...
        if action == "Create a new character":
            ...
            if provider == "BFL":
                ...
                base_image = generate_base_image_seedream4_replicate(base_prompt)
            elif provider == "SEEDREAM4_BYTEDANCE":
                base_image_id = generate_base_image_seedream4_bytedance(base_prompt)
                base_image = poll_image_seedream4_bytedance(base_image_id)
            else:
            ...
            for i, (prompt, instruct_prompt) in enumerate(zip(random_prompts, instruct_prompts)):
                try:
                    if provider == "BFL":
                        ...
                        i_image = edit_base_image_seedream4_replicate(base_image, instruct_prompt)
                    elif provider == "SEEDREAM4_BYTEDANCE":
                        i_image_id = edit_base_image_seedream4_bytedance(base_image, instruct_prompt)
                        i_image = poll_image_seedream4_bytedance(i_image_id)
                    else:
                        raise ValueError(f"Unknown provider: {provider}")
